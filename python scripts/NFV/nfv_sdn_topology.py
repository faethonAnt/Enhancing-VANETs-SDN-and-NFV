...
def topology():
    info("\n*** SDN + NFV VANET SCENARIO (with VNF monitor) ***\n")
    ...
    info("*** Creating backhaul (SDN switches + server + VNF)\n")
    srv = net.addHost('srv', ip='10.0.0.254/24')

    # NFV: VNF host for monitoring / QoS / analytics
    vnf = net.addHost('vnf1', ip='10.0.0.253/24')
    ...
    info("*** Adding bottleneck link agg1 <-> agg2 (same as SDN scenario)\n")
    ...
    # s1-eth4: -> vnf1 (NFV host)
    net.addLink(core, vnf, bw=50, delay='2ms', use_htb=True)
    ...
    info("*** Starting network with RemoteController (NFV scenario)\n")
    c0 = net.addController('c0', controller=RemoteController,
                           ip='127.0.0.1', port=6633)
    net.build()

    # Start APs and SDN switches (όλα προς c0)
    for ap in aps:
        ap.start([c0])
    agg1.start([c0])
    agg2.start([c0])
    ...
    info("*** Starting CLI (SDN + NFV mode)\n")
    CLI(net)
